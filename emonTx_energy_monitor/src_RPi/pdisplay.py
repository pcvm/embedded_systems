...
#
# Hardware setup: emonTxV3_4 running modified emonTxV3_4_continuous_kwhtotals
#                   |
#                   |(radio link)
#                   V
...
#    terminal emulator "cu" in a thread, accessing the RPI12Pi via a serial
...
import signal		# respond to external signals

#import keyboard	# respond to keypress
			# (not used as user must be root, do raw keyboard IO instead)
import tty
import termios
...
# Correct for small offset errors
# (this is added to sensor data to zero the display at midnight with no light e.g. moonshine, etc.)
offset_watts_totalGeneration = -11
...
# Allow external signals to set the quit flag
def signal_handler(signum, frame):
  global energyReportThread_quit_request2
  #print('Signal handler called with signal:', signum)
  energyReportThread_quit_request2=True
  print('*** Quit request set');
...
def data_collection_thread():
  ...
  #ssw_cnt=sys.stderr.write('Data collection thread starting\n')
  ...
  while True:
    ...
    if startup:
      ...
      #ssw_cnt=sys.stderr.write('*** First valid packet received\n')
      startup=False
    else:
    ...
    watts_totalImports    = dp_p[ id_totalImports ]
    watts_totalGeneration = dp_p[ id_totalGeneration ] + offset_watts_totalGeneration
    ...
    #
    #position = (0, 5*36)
    #text_surface = font.render('.'*25, True, (0,0,255))	# overwrite with blue
    #the_display.screen.blit(text_surface, position)
  ...
  #ssw_cnt=sys.stderr.write('*** Data collection thread has finished\n')
...
class adafruit_display:
    ...
    def __init__(self):
        ...
        if disp_no:
            print("Found X display set as {0}".format(disp_no))
        else:
            print("? No X display found so setting it as DISPLAY=:0")
            os.putenv('DISPLAY', ':0')
...
# Notes: 1) added the "su pi" as tty locking files were broken when run as root;
#        2) added the "sh -c" so that su runs sh which it trusts/likes, and not
#           cu which it will not run.
#cu_task= Popen('su pi sh -c "/usr/bin/cu -l '+serial_port+'"', shell=True, stdin=PIPE, stdout=PIPE, stderr=STDOUT, universal_newlines=True, close_fds=True)
...
Local commands:  q ==> quit; p ==> display latest powers
Send HUP signal:   ==> quit

''')

# save terminal state and enable unbuffered raw input (but keep output unchanged for raw_settings[1])
orig_settings = termios.tcgetattr(sys.stdin)
#tty.setraw(sys.stdin)
tty.setraw(sys.stdin)
raw_settings = termios.tcgetattr(sys.stdin)
raw_settings[1]=orig_settings[1]
termios.tcsetattr(sys.stdin, termios.TCSADRAIN, raw_settings)
print('Terminal input set to raw');
...
while True:
  ...
  while True:
    ...
    if energyReportThread_quit_request2: input='q'
    if len(input)>0 and ((input[0] == 'q') or (input[0] == 'v') or (input[0] == 'V') or (input[0] == 'p')): break
    print('-')
    time.sleep( 1 )
