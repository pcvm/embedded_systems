...
// likely to be 20% higher than 9V e.g. 9 x 1.2 = 10.8V. 
// In this case, the effective open circuit step down = 230 / 10.8 = 21.3V.
// As different countries can have variation in supply voltages (even for the
// same nominal value) and since transformers also display different open circuit
// characteristics, calibration constants are generated from macros that use
// measured data.

                                          // To obtain calibration specific to a
                                          // transformer, make measurements of the
                                          // mains rms and secondary rms voltages and
                                          // enter respective values:
#define Measured_Vrms_mains     (234)     //   (was entered as nominal 230)
#define Measured_Vrms_secondary (11.389)  //   (was estimated as 9*1.2=10.8 for "9V" transformer)
                                          // i.e. stepdown ratio =
                                          // Measured_Vrms_mains/Measured_Vrms_secondary

// The output voltage is then steped down further with the voltage divider which has 
// values Rb = 10k and Rt = 120k, which will reduce the voltage by 13 times.
//
// The combined step down is therefore 21.3 x 13 = 276.9 which is now represented by
// the theoretical calibration constant CombinedStepDown.
//
#define CombinedStepDown ( (13.0 * Measured_Vrms_mains) / Measured_Vrms_secondary ) // replaces 230 / 10.8 * 13 = 276.9
//
// As ADC_result==1023 corresponds to ADC_input==3.3V, then
//    ADC_result/1023 = ADC_input/3.3V and therefore
//    ADC_input = ADC_result/1023 * 3.3
// As ADC_input = external_V / CombinedStepDown, then we can obtain
// external_V = ADC_input * CombinedStepDown
//            = (ADC_result/1023*3.3) * CombinedStepDown
//            = ADC_result * CombinedStepDown * 3.3 / 1023
const float voltageScaling = CombinedStepDown * 3.3 / 1023;
...
const float powerCal_CT1 = (CombinedStepDown*(3.3/1023))*(90.9*(3.3/1023)); // <---- powerCal value
const float powerCal_CT2 = (CombinedStepDown*(3.3/1023))*(90.9*(3.3/1023)); // <---- powerCal value
const float powerCal_CT3 = (CombinedStepDown*(3.3/1023))*(90.9*(3.3/1023)); // <---- powerCal value
const float powerCal_CT4 = (CombinedStepDown*(3.3/1023))*(16.6*(3.3/1023)); // <---- powerCal value (2000 / 120R burden resistor)
...
{  
  ...
  Serial.println();
  Serial.println("----------------------------------");
  Serial.println("System ID: emonTxV3_4_continuous_kwhtotals_Vrms_Temp.ino");
  ...
  Serial.println ("Temperature display scaling: 100x");
  DS18B20.begin();
  Serial.print ("Temperature(C): ");
  DS18B20.requestTemperatures();    // take an initial sample and display
  tx_data.Temperature = 100.0*DS18B20.getTempCByIndex(0);
  Serial.print (tx_data.Temperature);
  Serial.println("/100");
  DS18B20.requestTemperatures();    // trigger next sample
  ...
  lastsendtime=millis();    // and initialise sendtime calculations
...
// the ISR is executed.  The ISR therefore needs to "look ahead" and this is managed as
// a state machine using variable sample_index to represent state N.
//
// At the end of ADC conversion for state N, ADC conversion for state N+1 will start automatically.
// The ISR which runs at this point therefore needs to capture the results of conversion for state N 
// and set up the conditions for conversion that follows state N+1 i.e. state N+2, and so on.  
// 
ISR(ADC_vect)  
{                                         
  ...
  {
    case 0:
      sample_V = ADC;		 	// retrieve sample_V, sample_CT1 in progress, so
      ADMUX = 0x40 + currentSensor_CT2; // set up next-but-one conversion for sample_CT2
      dataReady = true; 		// NOTE: sample_V is final A/D conversion, so flag dataReady
      sample_index++;			// advance state machine ID (means next interrupt triggers sample_CT1 read)
      break;
    case 1:
      sample_CT1 = ADC;			// retrieve data for state N, conversion for state N+1 in progress, so
      ADMUX = 0x40 + currentSensor_CT3; // set up next-but-one conversion for state N+2
      sample_index++;			// advance state machine ID (means next interrupt triggers state N+1 read)
      break;
    case 2:
      sample_CT2 = ADC; 
...
{ 
  ...
  {
    lastsendtime = lastsendtime + sendinterval; // Update not related to rereading millis()
...
void allGeneralProcessing()
{
  ...
  static int samplesDuringThisWindow_V;
      ...
      if (polarityOfLastSampleV != POSITIVE)
      {
        ...
        switch(sequenceCount)
        {
          ...
          case 60: 
            ...
            tx_data.realPower_CT3 = realPower_long * powerCal_CT3;
            
            joules_CT3 += tx_data.realPower_CT3 * 2;  // Joules elapsed in 100 cycles @ 50Hz is power J.s x 2 seconds
            
            tx_data.wh_CT3 += joules_CT3 / 3600;
          ...
          case 80: 
            ...
            tx_data.realPower_CT4 = realPower_long * powerCal_CT4;
            
            joules_CT4 += tx_data.realPower_CT4 * 2;  // Joules elapsed in 100 cycles @ 50Hz is power J.s x 2 seconds
            
            tx_data.wh_CT4 += joules_CT4 / 3600;
            ...
            Serial.print(':');
            Serial.print(tx_data.wh_CT4);          
            break;
          case 100:
            tx_data.Vrms = voltageScaling * sqrt( sumV_squared / samplesDuringThisWindow_V );
            sumV_squared = 0;
            samplesDuringThisWindow_V = 0;
  ...
  long sampleIminusDC_long_CT3 = ((long)(sample_CT3 - DCoffset_I))<<8;
  long sampleIminusDC_long_CT4 = ((long)(sample_CT4 - DCoffset_I))<<8;
  ...
  long  phaseShiftedSampleV_minusDC_long_CT3 = lastSampleV_minusDC_long
         + (((sampleV_minusDC_long - lastSampleV_minusDC_long)*phaseCal_int_CT3)>>8);  
  long  phaseShiftedSampleV_minusDC_long_CT4 = lastSampleV_minusDC_long
         + (((sampleV_minusDC_long - lastSampleV_minusDC_long)*phaseCal_int_CT4)>>8);  

  // The estimation of RMS voltage requires calculation of the sum of
  // (sample_V-offset)^2. RMS values are independent of phase.  The 10bit
  // ADC value in sampleV_minusDC_long has any offset removed and is left-
  // shifted by 8 so it occupies 18 bits. Right shifting by 8 to give Vtmp
  // means the square needs 2x10=20 bits which fits in 32bit maths.
  //
  // Precision check: summing 100 of these numbers needs another 7 bits, which
  //                  is OK as 24+7=31<32.
  // Scaling: the sampleV_minusDC_long>>8 means sample_V contributions unscaled.
  //
  long Vtmp = sampleV_minusDC_long >> 8;
  if (sample_V==0) Vtmp = 0;
  sumV_squared += Vtmp * Vtmp;
  samplesDuringThisWindow_V++;
