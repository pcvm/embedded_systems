...
#define ThisRelease "2017 r11"	// 10. some sensor attributes are concatenated
				//     to give a final My_Version C-string
				// 11. option to specify time server
				//
				
const unsigned char OptSep = 0xff;
const char          OptTimeServer = 't';
...
bool retrieve_config_OK() {
  ...
  // - currently only implement single case of option OptTimeServer
  if (misc_bytes[1]==OptTimeServer)
...
void save_config(char *s, char *p, signed char ttz, char *option_types, char *option_values) {
  EEPROM.begin(EEPROM_nBytesUsed);
  delay(10);
...
void handleUpdate() {
  ...
  String opt_TimeServer = server.arg("t");
  ...
  char *ordered_options = (char*) (String("") + OptTimeServer).c_str();
  save_config(ssid_ptr, pwd_ptr, temp_byte, ordered_options, ordered_values);
  ...
  // system is restarting so just use system delay() calls for delay
#if defined( use_7SEGMENT_DISPLAY )
...
void setup() {
  ...
  Serial.println(" LCD");			//    (LCD setup so allPrint* functions available)
