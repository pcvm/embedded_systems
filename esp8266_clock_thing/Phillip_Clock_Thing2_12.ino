...
#define ThisRelease "2018 r12"	// r10. some sensor attributes are concatenated
...
void reconfigure_system_via_serial_port() {
  ...
  clear_lcd_row(0);
  Serial.println("Restart necessary.");
  Serial.println("Please power down the clock, wait a few seconds, and then power it up again.");
  Serial.println();
  Serial.println();
  Serial.println();
...
void setup() {
  ...
  if (Serial.available() > 0) {
    Serial.println();
    Serial.println("Entering reconfigure mode using serial communications");
    reconfigure_system_via_serial_port();	// never return
  }    

  if ((digitalRead(theButton) == theButtonActive) || !retrieve_config_OK()) {
    Serial.println("Starting access point for reconfiguration");
    reconfigure_system_via_local_WIFIap();	// never return
  } else {
    Serial.println();
    Serial.println();
