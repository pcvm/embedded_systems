 ...
 *         time, and temp&humidity is read from a DHT sensor.
 *
 ...
 * 10 Add timezone data to eeprom storage (now holds 2 strings and an int), and test GPIO16 ==1 at power up
 *    for user reset option whereby the device enables its local access point and inputs config data
 */
...
LedControl lc=LedControl(SPI_mosi, SPI_sck, SPI_ss, numDevices);  // Note: miso unused
...
void setup()
{
  ...
  //Serial.begin(115200);       // setup serial comms if using serial comms for debugging
  ...
  lc_setup();                   // initialise LED matrix
  ...
  pinMode( theButton, INPUT );  // user reset button
  delay(250);
  // If user reset == HIGH, directly call reconfigure_system_via_local_WIFIap()
