 ...
 *         with previous config stored in EEPROM. A 4 digit LED output is also provided.
 *
...
const int nDigits    = 11;  // ' ' char is not bit reversed
const int nChars     = 12;
...
LedControl lc=LedControl(13,14,15,numDevices);
...
void lc_setup()
{
  for (int x=0; x<numDevices; x++){
    ...
    lc.clearDisplay(x);         // and clear the display
                                ...
                                // reverse the char pixel scans through the Y axis
                                // [we could avoid this at run-time but RAM is abundant]
  ...
  for (int c=0;c<nDigits; c++)                // for each digit that is to be bit reversed...
  {
    ...
    for (int s=0;s<n_vPixels_per_digit; s++)  // for each bit...
    {
      uint8 tmp = 0;
      for (int i=0;i<n_hPixels_per_digit;i++) // test original bits R-to-L, to control 1/0 shift in R-to-L
        if ((1<<i) & *(this_digit+s)) tmp = (tmp << 1) | 1;
        else                          tmp = tmp << 1;
                                // perform xtra left shifts to center char,
                                // based on char width in digits[c][n_vPixels_per_digit]
...
//   fed from top to bottom into the aray columns
//
void display_dotMatrix4digits( int dLeft, int dRight )
{
      uint8* ptr;
      int tmp[4];
  ...
  if ( clock_tick>=Timer_IRQs_per_second )
    {
      ...
      display_dotMatrix4digits( dt_hours, dt_mins );// update LED
